...
const { plans, currentPlan, planning, agentStatuses, generatePlan, selectPlan, toggleTask, deletePlan } = usePlan()

const showSettings = ref(false)
const activeView = ref<'chat' | 'plan'>('chat')
const toast = ref('')
...
async function handleGeneratePlan(goal: string) {
  ...
  try {
    ...
    toast.value = err instanceof Error ? err.message : '规划生成失败'
    setTimeout(() => (toast.value = ''), 4000)
  }
}

function handleDelete(planId: string) {
  if (confirm('确定删除这份规划吗？')) {
    deletePlan(planId)
  }
}
</script>
...
<template>
  <div class="min-h-dvh flex flex-col">
    ...
    <Transition name="fade">
      <div
        v-if="toast"
        class="fixed top-20 left-1/2 -translate-x-1/2 z-50 px-4 py-2 bg-slate-800 text-white text-sm rounded-lg shadow-lg"
      >
        {{ toast }}
      </div>
    </Transition>
    ...
    <main class="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 py-4 sm:py-6 flex flex-col min-h-0">
      <nav class="shrink-0 flex gap-1 mb-4 p-1 bg-white/60 rounded-xl w-fit">
        <button
